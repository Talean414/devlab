...
export const AI_PROVIDER_PROFILES: AiProviderProfile[] = [
  ...
  },
  {
    ...
    transport: "Rust-owned HTTPS adapter to the fixed host api.deepseek.com (bounded prompt and reply, 120 s bound, streamed or single reply)",
  ...
  },
  {
    ...
    note: "Phase 9A native local adapter (Phase 9D adds embeddings for semantic workspace search through the same adapter). Only http://localhost or 127.0.0.1/::1 endpoints are accepted, the WebView never calls localhost itself, and replies are bounded (streamed or single). Unavailable in the plain web preview.",
  },
  {
    id: "openai",
    name: "OpenAI",
    ...
    availableNow: NATIVE_RUNTIME,
    credentialStorage: "OS credential store entry written through Rust; never returned to the WebView",
    transport: "Rust-owned HTTPS adapter to the fixed host api.openai.com (bounded prompt and reply, 120 s bound, streamed or single reply)",
  ...
  },
  {
    ...
    availableNow: NATIVE_RUNTIME,
    credentialStorage: "OS credential store entry written through Rust; never returned to the WebView",
    transport: "Rust-owned HTTPS adapter to the fixed host api.anthropic.com (bounded prompt and reply, 120 s bound, streamed or single reply)",
  ...
  },
  {
    ...
    transport: "Rust-owned bounded HTTP adapter with an explicit host policy (https:// required unless loopback; fixed /chat/completions suffix; 120 s bound, streamed or single reply)",
    defaultModel: "",
