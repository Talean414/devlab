...
- Phase 9A delivers local Ollama support through a Rust-owned loopback-only adapter with explicit endpoint configuration, bounded timeouts, honest connection errors and no implicit network fallback; streaming replies and per-task model selection remain future work.
