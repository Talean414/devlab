...
Phase 7A implements the first inline Monaco diff review inside the reviewed-draft Editor gate, with original workspace content on the left and reviewed draft content on the right. The deeper polish below remains future work.

- Continue improving side-by-side file comparison with richer inline decorations, line-level navigation and per-file apply state directly where the developer is reviewing code.
