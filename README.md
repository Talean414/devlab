...
Phase 6X implements the first bounded metadata-only repository map by recursively listing scoped workspace paths without reading file contents. Phase 8F adds a client-side preview/filter/copy inspector over the already-attached metadata-only map. The structural/semantic layers below remain future work.
