...
- Reviewed one-file repair drafts generated from real failing test evidence and opened in the editor draft-review flow without automatic writes
...
Phase 5 is complete through Docker, SQLite, PostgreSQL and native HTTP. Phase 6 has started with bounded native test execution and reviewed one-file repair drafts; broader agent patch tools and signed distribution follow afterward. Until a backend exists, DevLab reports that the feature is unavailable instead of fabricating data or success.
...
## 12. Run native workspace tests

1. Select the workspace you want to test through **Code Editor**.
2. Open **Self-Healing Tests / Native Test Runner**. In Phase 6B this panel runs real tests and can draft a one-file repair for review; it still does not write patches automatically.
...
7. Review the draft manually in **Code Editor**. To persist it, copy or apply only the changes you approve; DevLab does not write the repair draft automatically.

Each run has a 60-second process timeout and captures at most 2 MiB per output stream. Repair prompts include at most 64 KiB of the selected source file and bounded excerpts of stdout/stderr. DevLab sets `CI=1`, `NO_COLOR=1` and `TERM=dumb`, uses the selected workspace as the working directory, and kills the owned process tree on timeout. Project test scripts still execute with the operating-system authority of the user running DevLab, so treat them like terminal commands from that repository.

## 13. Verify the complete build
...
- [ ] A failed native test run can generate a reviewed one-file repair draft from a selected existing source file; the draft opens in editor review and nothing is written automatically
...
- Keep repair drafts review-only: base them on real native test output, read one existing workspace file through the native boundary, bound prompt/source size, and open the generated file as an in-memory editor draft rather than writing it automatically.
