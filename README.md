...
3. The renderer turns the result into markdown — one heading per file, one line per symbol (`kind *name — signature (Lstart–end)`) — and emits whole files in order until the context budget would be exceeded, then reports how many files were omitted instead of cutting mid-entry. It attaches as synthetic read-only context (`devlab-code-map.md`, source `code-map`) within the same four-item / 48 KiB budget as files, outlines and the repository map; Refresh rebuilds it with the same mode. It is memory-scoped, excluded from recovery snapshots and not recorded as a file-content audit event.
4. Nothing is cached or written; each attach or refresh is a fresh bounded build. File bodies never leave Rust — the map carries names, kinds, first-line signatures and line ranges only.
