...
Phase 6Y implements the first Builder-side spec review pack by deriving a bounded `spec.md` and sequential task DAG from an approved plan, then keeping it in memory until copy or reviewed Editor staging. Phase 8H enriches that generated spec with metadata-only acceptance criteria, dependency assumptions, risk notes and review gates. Phase 8I adds a read-only preview/copy affordance for that spec metadata before `spec.md` is copied or staged. The deeper agent-run orchestration below remains future work.
