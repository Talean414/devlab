...
- Detect available project checks from trusted profiles such as `tsc --noEmit`, `cargo check`, `biome check`, `eslint`, `pytest`, Go tests or package-manager scripts. Phase 9S delivers the first batch as backend-owned **check** profiles (package.json typecheck/lint/check scripts, direct `tsc --noEmit` when installed, `cargo check`/`cargo clippy --no-deps`, `go vet`, `ruff check`, `mypy`); direct `eslint`/`biome` invocations without a package script remain future work.
...
Phase 7A implements the first inline Monaco diff review inside the reviewed-draft Editor gate, with original workspace content on the left and reviewed draft content on the right. Phase 7B adds read-only Monaco change-block counts, Previous/Next navigation and visible per-file apply state in that same review surface. Phase 7C adds session-only review annotations and bounded reviewer notes for each generated draft. Phase 7D adds session-only review-queue filters for all, pending, applied, reviewed and needs-changes drafts. Phase 7E adds keyboard shortcuts for read-only diff navigation and filtered review-queue movement. Phase 8S adds session-only saved review views so a reviewer can name, restore, delete or copy a snapshot of the current filter, annotations and selection for one queue. Phase 9T adds hunk-level accept/reject: the reviewer keeps or drops each Monaco change block (per-hunk buttons, a reset action, or Alt+A/Alt+R at the cursor), and the in-memory composed file — kept blocks from the generated draft plus original lines for dropped blocks — is written only through the same explicit Apply reviewed draft button. The deeper polish below remains future work.

- Continue improving side-by-side file comparison with richer inline decorations and finer character-level review affordances beyond the current change-block navigation, hunk-level accept/reject with in-memory composition, session-only annotations, review-queue filters, saved review views and keyboard shortcuts.
...
- Add PR/branch orchestration only after GitHub/GitLab operations, CI status reads and write actions are bounded, audited and confirmation-gated. Phase 9R keeps the Deploy and CI / CD panels authoring-only in the meantime (recipes and workflow templates staged as reviewed drafts, no tokens, no execution, no run status).

### Additional future upgrades

- Phase 9P delivers the Ollama half of the local model health check (see 13b′): version, installed/loaded state, context windows, embedding sizes and capabilities read from the loopback API without loading or prompting a model. Health checks for other local inference servers behind the custom OpenAI-compatible adapter remain future work.
...
- Phase 9Q delivers the review-time half of migration safety planning (see 12b): lexical lock-risk warnings, rollback notes and an environment-separation checklist for every SQL migration draft before it can be applied. A real dry run (executing the migration inside a rolled-back transaction against a staging database through the native PostgreSQL session) remains future work and would need its own confirmation gate.
- Phase 9L delivers the module-level dependency graph behind the planned architecture diagrams (imports resolved per language, aggregated into directory modules with weighted edges and external packages) Phase 9M renders it as a Mermaid diagram plus a ranked architecture summary for Agent context, and Phase 9O draws that diagram inside the Agent panel as a dependency-free layered SVG (zoom, neighbour highlighting, per-module details, Mermaid copy); combining it with the repository map into one interactive picture remains future work.
...
- Lexical migration safety notes (lock risks, rollback notes, environment checklist) for SQL drafts before reviewed apply
...
- Hunk-level accept/reject for reviewed drafts: keep or drop each Monaco change block and apply the in-memory composed file through the same explicit editor-side apply button
...
- Optional local session recovery prompt for Agent and Project Builder progress, stored in WebView localStorage until continued or cleared, with a remembered resume-or-start-fresh preference changeable in Settings → Interface
- Basic/Advanced interface mode: an essentials-only sidebar, Settings surface and Builder view for everyday work, with the full tooling surface one toggle away
...
## 12b. Review migration safety notes before applying a SQL draft

1. Generate a migration in **Natural-Language Migrations** and open the **SQL migration** tab, or stage any draft whose path ends in `.sql` or lives under a migration folder (`prisma/migrations/`, `migrations/`, `db/migrate/`, `alembic/versions/`, `supabase/migrations/`, …) for Editor review.
2. DevLab splits the draft into statements (respecting comments, string literals and `$$` bodies), detects the dialect from the SQL or the accompanying `schema.prisma`, and lists findings by severity: **danger** for data loss or long exclusive locks (DROP/TRUNCATE, full-table UPDATE/DELETE, `ADD COLUMN … NOT NULL` without a default, `ALTER COLUMN … TYPE`, `CREATE INDEX CONCURRENTLY` inside a transaction), **warning** for blocking-but-recoverable steps (non-concurrent index builds, foreign keys or CHECK constraints without `NOT VALID`, `SET NOT NULL`, volatile defaults, renames, explicit `LOCK TABLE`, `VACUUM FULL`) and **info** for additive metadata-only changes.
3. Each statement gets a rollback note: a suggested inverse (`DROP COLUMN IF EXISTS`, `DROP INDEX CONCURRENTLY IF EXISTS`, `DROP CONSTRAINT`, reverse `RENAME`) or an explicit "not reversible from the schema; restore from backup" marker for drops, truncates, data changes and type changes.
4. The checklist covers environment separation: rehearse on a staging database restored from a production snapshot, take a verified backup first, deploy in expand/contract order, set `lock_timeout` before DDL, keep backfills out of the schema transaction, decide explicitly how the migration runner wraps transactions, and keep production credentials out of the workspace.
5. **Copy notes** exports the whole review as markdown for a PR description or runbook. The review is a lexical read of the text — it is **not a dry run**: DevLab does not execute the SQL, does not contact a database and cannot know table sizes, so rehearsal on staging is still required. Apply remains behind the same explicit reviewed-draft button.

## 12c. Author deploy recipes and CI workflow templates (authoring only)

1. **Deploy** lists the supported providers (Vercel, Netlify, Render, Railway, Fly.io, Cloudflare, Supabase, GitHub Pages) with CLI install, CLI login, deploy commands, the secret name a CI job expects (`VERCEL_TOKEN`, `NETLIFY_AUTH_TOKEN`, `FLY_API_TOKEN`, …) and a GitHub Actions workflow starting point. DevLab does not ask for a deployment token: there is no protected native storage for deployment credentials yet, so log in with the provider CLI in your terminal and configure CI secrets in the CI provider. The only persisted value is the non-secret default provider id.
2. **CI / CD** lists DevLab's catalog of workflow templates (Node CI, Docker build and push, GitHub Pages, Python CI). The run-status area says **Not connected**: DevLab does not read GitHub Actions or any other provider's runs in this checkpoint and never shows sample runs; use the provider dashboard or `gh run list` / `gh run watch` from the integrated terminal.
3. **Stage for review** in either panel opens the workflow file (for example `.github/workflows/deploy-vercel.yml` or `.github/workflows/node-ci.yml`) through the same reviewed-draft gate as Project Builder: the path policy runs first, Rust validates the draft metadata, and the file is written only when you click Apply in the Editor. Committing and pushing the workflow stays with you — DevLab does not push workflows, run deployments or execute any of the listed commands.
...
3. Click **Refresh profiles**. Rust re-discovers supported profiles from the selected workspace: package.json test-like scripts, `Cargo.toml`, `go.mod`, pytest config files or a `tests/` directory. Since Phase 9S it also lists non-mutating **check** profiles (badged *check*): package.json `typecheck`, `lint`, `check` and `format:check`-style scripts (scripts containing `fix` or `write`, plain `format`/`prettier`, and npm pre/post hooks are never offered), `tsc --noEmit` when `tsconfig.json` exists and `node_modules/typescript` is installed, `cargo check` and `cargo clippy --no-deps`, `go vet ./...`, and `ruff check .` / `mypy .` when a ruff or mypy configuration is present. Checks sort after tests; a missing tool (for example the clippy component) is reported as a failed run, never installed.
...
## 13b′. Check local model health (native only)

1. In Settings → Providers → **Ollama (local)**, click **Check model health** (beside Detect installed models).
2. Rust (`ollama_model_health`) talks only to the validated loopback endpoint and only to fixed paths: `GET /api/version` (server version, best effort), `GET /api/tags` (installed models — the reachability check), `GET /api/ps` (loaded models with VRAM size, expiry and running context length; if it fails the loaded state is reported as unknown rather than "not loaded") and one `POST /api/show` per installed model (up to 12, 15 s each, 45 s in total) for the trained context window (`<architecture>.context_length`), a Modelfile `num_ctx` when the model sets one, embedding size, parameter count, quantization, format and the server's capability list (`completion`, `tools`, `embedding`, …). Whichever bound trips first is reported as `truncationReason`.
3. Every model shows its loaded state, its context window summary — for example `max 128k tokens · running with 4k` — and its metadata; a per-model probe failure is shown on that card while the rest of the check completes. The configured chat and embedding models are labelled, and DevLab warns when a configured model is not installed (with the matching `ollama pull` command) or does not report the capability its role needs.
4. Nothing is loaded, pulled, prompted or written: the check reads metadata only, so it cannot tell you how fast a model generates — that is what the existing chat path is for. Results live in Settings memory and are not persisted.
...
## 13c⁗⁗′. Check custom endpoint health (native only)

1. In Settings → Providers → **Custom OpenAI-compatible endpoint**, click **Check server health** beside the base URL and token controls.
2. Rust (`custom_endpoint_health`) re-validates the base URL with the same host policy, attaches the profile-scoped bearer token when one is stored (only over `https://`), and sends a single read-only `GET <base>/models` through the bounded Rust client (20 s bound); the WebView never opens the connection.
3. The reply is parsed with the OpenAI list shape and bounded: at most 200 entries, model ids capped at 256 bytes, owners capped, created timestamps best-effort. Listed models are shown, and clicking one sets it as the custom endpoint model id (an explicit preference, like the Ollama model list).
4. Failures are reported honestly: unreachable/DNS/timeout/TLS errors carry the same `ai_provider_*` codes as the chat path; 401/403 means the stored token is missing or wrong; a server that answers but does not expose `GET /models` (HTTP 404/405) is a **warning** — the chat completions path may still work — not a failure; and a configured model id that is not in the list is flagged, since some gateways only list a subset.
5. Nothing is loaded, pulled, prompted or written: the check reads metadata only, results live in Settings memory and are not persisted, and changing the endpoint URL discards the last result.
...
4. After **Check model health** in the Ollama section, the router shows **Local model suggestions**: for each task class the best-fit installed Ollama models, ranked (top 3) from the reported context window (trained maximum, Modelfile `num_ctx` cap, running value), parameter size and loaded state, with task-class-specific weights (chat also rewards snappier models; reasoning and implementation reward depth; vision stays on Gemini). Suggestions are metadata only, derived from the last health check and the current routing — nothing is switched automatically; applying one is the explicit per-task provider/model change above.
...
5. Phase 9O also draws the diagram in the Agent panel as soon as the summary is attached: an **Architecture diagram** inspector renders the same modules and edges as the Mermaid text as an inline SVG laid out left to right (entry points on the left, shared libraries on the right; cycles are broken for layering and their closing edges drawn dashed in amber). Zoom between 50 % and 200 %, click a box to highlight its neighbours and read its file count, importance, in-repo edges, dependencies, dependants and external packages, and use **Copy Mermaid source** to paste the diagram into any Mermaid renderer. The drawing is computed in memory from the attached graph — no charting library, no network, no cache, nothing written — and disappears when the architecture context is removed.
...
- [ ] Deployment panel says **Native deployment is disabled**, asks for no token and only stages workflow recipes for Editor review; CI / CD shows **Not connected** instead of sample runs; Database clearly marks a read-only PostgreSQL session and shows the write state of every connection
